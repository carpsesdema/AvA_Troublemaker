...
try:
    ...
    def get_gemini_api_key(): return APP_CONFIG.get("GEMINI_API_KEY")
    def get_openai_api_key(): return os.getenv("OPENAI_API_KEY")
except ImportError:
    def get_gemini_api_key(): return os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    def get_openai_api_key(): return os.getenv("OPENAI_API_KEY")
    logging.warning("ChatManager: config.py's APP_CONFIG or API key getters not found. Using environment variables as fallback.")
...
class ChatManager(QObject):
    ...
    def __init__(self, orchestrator: ApplicationOrchestrator, parent: Optional[QObject] = None):
        ...
        if not isinstance(orchestrator, ApplicationOrchestrator):
            ...
            logger.critical(err_msg); raise TypeError(err_msg)
    ...
    def _initialize_state_variables(self):
        ...
        if DEFAULT_CHAT_BACKEND_ID in self._current_model_names: self._current_model_names[DEFAULT_CHAT_BACKEND_ID] = DEFAULT_GEMINI_CHAT_MODEL
        if OLLAMA_CHAT_BACKEND_ID in self._current_model_names: self._current_model_names[OLLAMA_CHAT_BACKEND_ID] = "llama3:latest"
        if GPT_CHAT_BACKEND_ID in self._current_model_names: self._current_model_names[GPT_CHAT_BACKEND_ID] = DEFAULT_GPT_MODEL
        if PLANNER_BACKEND_ID in self._current_model_names: self._current_model_names[PLANNER_BACKEND_ID] = DEFAULT_GEMINI_PLANNER_MODEL
        if GENERATOR_BACKEND_ID in self._current_model_names: self._current_model_names[GENERATOR_BACKEND_ID] = DEFAULT_OLLAMA_MODEL

        self._current_chat_personality_prompts: Dict[str, Optional[str]] = { bid: None for bid in all_backend_ids_from_adapters }
        if PLANNER_BACKEND_ID in self._current_chat_personality_prompts: self._current_chat_personality_prompts[PLANNER_BACKEND_ID] = "You are an expert planner and technical writer."
        if GENERATOR_BACKEND_ID in self._current_chat_personality_prompts: self._current_chat_personality_prompts[GENERATOR_BACKEND_ID] = "You are an expert Python coding assistant. Your responses must be only code, in a single markdown code block."

        self._current_chat_temperature: float = 0.7
        self._chat_backend_configured_successfully: Dict[str, bool] = { bid: False for bid in all_backend_ids_from_adapters }
        ...
        self._rag_available: bool = (self._vector_db_service is not None and hasattr(self._vector_db_service, 'is_ready') and self._vector_db_service.is_ready())
    ...
    def initialize(self):
        ...
        if not (self._session_flow_manager and self._project_context_manager and self._user_input_handler and self._backend_coordinator):
            missing_deps = [name for comp, name in [ (self._session_flow_manager, "SFM"), (self._project_context_manager, "PCM"), (self._user_input_handler, "UIH"), (self._backend_coordinator, "BC")] if not comp]
            logger.critical(f"Cannot initialize ChatManager: Critical components missing: {missing_deps}")
            self.error_occurred.emit(f"Critical error during init ({', '.join(missing_deps)} missing).", True); return
        ...
        else: logger.error("CM Init: SessionFlowManager.load_last_session_state_on_startup returned invalid data. Using defaults.")
        ...
        if active_backend_id_from_session and active_backend_id_from_session in self._current_model_names:
            ...
            if pers_from_session: self._current_chat_personality_prompts[self._current_active_chat_backend_id] = pers_from_session
        ...
        if generator_model_from_session and GENERATOR_BACKEND_ID in self._current_model_names: self._current_model_names[GENERATOR_BACKEND_ID] = generator_model_from_session

        if proj_data_from_session: self._project_context_manager.load_state(proj_data_from_session)
        ...
        logger.info(f"ChatManager late init complete. Active project: {current_active_project_id}. Active Chat Backend ID: {self._current_active_chat_backend_id}.")
    ...
    def _configure_all_initial_backends(self):
        ...
        for backend_id in self._backend_adapters_dict.keys():
            ...
            if not model_to_use:
                ...
                else: model_to_use = "default_model_placeholder"
            ...
            self._backend_coordinator.configure_backend(backend_id, api_key_for_this_backend, model_to_use, personality_to_use)
        ...
        for provider_detail in USER_SELECTABLE_CHAT_BACKEND_DETAILS:
            ...
            try:
                for model_name_from_adapter in self.get_models_for_backend(backend_id):
                    all_models_details.append({"display_name": f"{provider_name}: {model_name_from_adapter}", "backend_id": backend_id, "model_name": model_name_from_adapter})
            except Exception as e: logger.error(f"Error fetching/processing models for chat backend {backend_id}: {e}")
    ...
    def get_all_available_specialized_models_with_details(self) -> List[Dict[str, Any]]:
        ...
        try:
            for model_name_from_adapter in self.get_models_for_backend(GENERATOR_BACKEND_ID):
                all_models_details.append({"display_name": f"Ollama (Specialized): {model_name_from_adapter}", "backend_id": GENERATOR_BACKEND_ID, "model_name": model_name_from_adapter})
        except Exception as e: logger.error(f"Error fetching/processing models for specialized backend {GENERATOR_BACKEND_ID}: {e}")
        return all_models_details

    def get_available_backend_details(self) -> List[Dict[str, str]]:
        return [detail for detail in USER_SELECTABLE_CHAT_BACKEND_DETAILS if detail['id'] in self._backend_adapters_dict]
    ...
    def get_models_for_backend(self, backend_id: str) -> List[str]:
        ...
        if self._backend_coordinator:
            try:
                ...
                return models
            except Exception as e: logger.exception(f"Error fetching models for backend '{backend_id}' via BackendCoordinator:")
    ...
    def _handle_sfm_session_loaded(self, model_name: str, personality: Optional[str], proj_ctx_data: Dict[str, Any], active_pid_from_session: str):
    ...
    def _handle_sfm_active_history_cleared(self):
        if self._project_context_manager:
            ...
            if active_project_id and (history := self._project_context_manager.get_project_history(active_project_id)):
                history.clear(); self.history_changed.emit([])
    ...
    @pyqtSlot(str)
    def _handle_backend_stream_started(self, request_id: str): self.stream_started.emit(request_id)

    @pyqtSlot(str, str)
    def _handle_backend_chunk_received(self, request_id: str, chunk: str):
        if not (self._modification_coordinator and self._modification_coordinator.is_awaiting_llm_response()):
    ...
    def _handle_backend_response_completed(self, request_id: str, completed_message: ChatMessage, usage_stats_with_metadata: dict):
        ...
        if self._modification_coordinator and self._modification_coordinator.is_active() and is_mc_related_purpose:
            ...
            elif (hasattr(ModPhase, 'AWAITING_GEMINI_REFINEMENT') and self._modification_coordinator._current_phase == ModPhase.AWAITING_GEMINI_REFINEMENT and backend_id_for_mc == PLANNER_BACKEND_ID): mc_is_expecting_this = True
            if mc_is_expecting_this: self._modification_coordinator.process_llm_response(backend_id_for_mc or PLANNER_BACKEND_ID, completed_message); return
            else: logger.warning(f"CM: Response for req_id '{request_id}' has MC purpose ('{purpose}') but MC is not in a matching await state. This response will NOT be displayed in chat."); return
        if purpose == "code_summary" and (original_target_filename := usage_stats_with_metadata.get("original_target_filename")):
        ...
        if self._project_context_manager and (active_history := self._project_context_manager.get_active_conversation_history()):
        ...
        prompt_tokens, completion_tokens = usage_stats_with_metadata.get("prompt_tokens"), usage_stats_with_metadata.get("completion_tokens")
        backend_id_for_tokens = backend_id_for_mc if backend_id_for_mc else (GENERATOR_BACKEND_ID if "technical_summary" in (purpose or "") else (DEFAULT_CHAT_BACKEND_ID if "friendly_summary" in (purpose or "") else self._current_active_chat_backend_id))
    ...
    def _handle_backend_response_error(self, request_id: str, error_message_str: str):
        ...
        if self._project_summary_coordinator and ( (hasattr(self._project_summary_coordinator, '_current_request_id_tech_summary') and self._project_summary_coordinator._current_request_id_tech_summary == request_id) or (hasattr(self._project_summary_coordinator, '_current_request_id_friendly_summary') and self._project_summary_coordinator._current_request_id_friendly_summary == request_id) ):
            is_psc_related_error = True
        if is_psc_related_error: self.stream_finished.emit(); return
        if self._modification_coordinator and self._modification_coordinator.is_active() and request_id.startswith("mc_"):
            self.stream_finished.emit(); return
        message_updated_in_model = False
        if self._project_context_manager and (active_history := self._project_context_manager.get_active_conversation_history()):
            for msg_in_history in reversed(active_history):
                if msg_in_history.id == request_id and msg_in_history.role == MODEL_ROLE:
        ...
        if self._project_context_manager: self._trigger_save_last_session_state()
        self.stream_finished.emit(); self.error_occurred.emit(f"Backend Error: {error_message_str}", False)

    @pyqtSlot(bool)
    def _handle_backend_busy_changed(self, backend_is_busy: bool): self._update_overall_busy_state()
    ...
    def _handle_backend_configuration_changed(self, backend_id: str, model_name: str, is_configured: bool, available_models: list):
        ...
        self.backend_config_state_changed.emit(backend_id, model_name, is_configured, bool(self._current_chat_personality_prompts.get(backend_id)))
        if backend_id == self._current_active_chat_backend_id: self.update_status_based_on_state()
            ...
            self.status_update.emit(status_msg, "#98c379" if is_configured else "#e06c75", True, 3000 if is_configured else 5000)
        self._trigger_save_last_session_state()

    @pyqtSlot(bool, str)
    def _handle_upload_started(self, is_global: bool, item_description: str):
        active_project_name_str = (self._project_context_manager.get_active_project_name() or "Current") if self._project_context_manager else "N/A"
    ...
    def _handle_upload_summary(self, summary_message: ChatMessage):
        if not self._project_context_manager: return
        self._project_context_manager.add_message_to_active_project(summary_message); self.new_message_added.emit(summary_message)
    ...
    def _handle_uih_normal_chat_request(self, new_user_message_list: List[ChatMessage]):
        if not (self._backend_coordinator and self._project_context_manager): self.error_occurred.emit("Cannot send chat: Critical components missing.", True); return
        ...
        self._project_context_manager.add_message_to_active_project(user_message_for_ui); self.new_message_added.emit(user_message_for_ui)
        self._trigger_save_last_session_state(); QApplication.processEvents()
        ...
        self._project_context_manager.add_message_to_active_project(ai_placeholder_message); self.new_message_added.emit(ai_placeholder_message)
    ...
    def _handle_uih_mod_start_request(self, original_query_text: str, image_data_list: List[Dict[str, Any]], context_for_mc: str, focus_prefix_for_mc: str):
        ...
        self._modification_coordinator.start_sequence(query=original_query_text, context_from_rag=context_for_mc, focus_prefix=focus_prefix_for_mc)
    ...
    def _handle_mc_request_llm_call(self, target_backend_id: str, history_to_send: List[ChatMessage]):
        if self._backend_coordinator:
            mc_options = {"temperature": 0.2} if target_backend_id == GENERATOR_BACKEND_ID else {"temperature": 0.5}
    ...
    def _handle_code_generated_and_summary_needed(self, generated_code: str, coder_instructions: str, target_filename: str):
        ...
        if not self._code_summary_service.request_code_summary(self._backend_coordinator, target_filename, coder_instructions, generated_code):
            if self._project_context_manager:
                sys_err_msg = ChatMessage(role=ERROR_ROLE, parts=[f"[System: Error initiating summary for '{target_filename}'.]"]); self._project_context_manager.add_message_to_active_project(sys_err_msg); self.new_message_added.emit(sys_err_msg)
            self.update_status_based_on_state()
    ...
    def _handle_mc_file_ready(self, filename: str, content: str):
        ...
        if self._project_context_manager:
            sys_msg = ChatMessage(role=SYSTEM_ROLE, parts=[f"[System: File '{filename}' updated. See Code Viewer.]"], metadata={"is_internal": False}); self._project_context_manager.add_message_to_active_project(sys_msg); self.new_message_added.emit(sys_msg)

    @pyqtSlot(str, str)
    def _handle_mc_sequence_complete(self, reason: str, original_query_summary: str):
        logger.info(f"CM: Modification sequence complete. Reason: '{reason}', Query Summary: '{original_query_summary}'.")
        if self._project_context_manager:
            ...
            logger.info(f"CM: Adding strengthened internal system message: {strengthened_system_message_text}")
            internal_sys_msg = ChatMessage(role=SYSTEM_ROLE, parts=[strengthened_system_message_text], metadata={"is_internal": True})
            ...
            if reason.startswith("completed"):
                ...
                if reason == "completed_no_files": priming_model_message_text = f"Okay, it looks like no changes were actually needed for '{original_query_summary}'. What can I help you with now? ✨"
                elif reason == "completed": priming_model_message_text = f"Alright, all done with the '{original_query_summary}' task! What's next on your list? 🚀"
                else: priming_model_message_text = f"The task '{original_query_summary}' has finished ({reason}). What shall we do next?"
                if priming_model_message_text:
                    logger.info(f"CM: Adding priming model message: {priming_model_message_text}")
                    ...
                    self.new_message_added.emit(priming_model_msg) # Ensure this is emitted for the user to see
            self._trigger_save_last_session_state()
        self.update_status_based_on_state()
        if self._modification_handler_instance: self._modification_handler_instance.cancel_modification()
        logger.info(f"CM: Modification sequence handling complete. Ava should now be in conversational mode.")

    @pyqtSlot(str)
    def _handle_mc_error(self, error_message: str):
        if self._project_context_manager:
            err_msg_obj = ChatMessage(role=ERROR_ROLE, parts=[f"Modification System Error: {error_message}"], metadata={"is_internal": False}); self._project_context_manager.add_message_to_active_project(err_msg_obj); self.new_message_added.emit(err_msg_obj); self._trigger_save_last_session_state()
    ...
    def _handle_mc_status_update(self, message: str):
        if self._project_context_manager:
            status_msg = ChatMessage(role=SYSTEM_ROLE, parts=[message], metadata={"is_internal": False}); self._project_context_manager.add_message_to_active_project(status_msg); self.new_message_added.emit(status_msg)
    ...
    def _handle_project_summary_generated(self, project_id: str, friendly_summary_text: str):
        ...
        summary_chat_message = ChatMessage(role=MODEL_ROLE, parts=[f"✨ **Ava's Project Insights for '{project_name}'!** ✨\n\n{friendly_summary_text}"], metadata={"is_project_summary": True, "project_id": project_id, "is_internal": False})
        if (target_history := self._project_context_manager.get_project_history(project_id)) is not None:
            ...
            if self._project_context_manager.get_active_project_id() == project_id: self.new_message_added.emit(summary_chat_message)
            else: self.status_update.emit(f"Project summary for project '{project_name}' is ready!", "#98c379", True, 5000)
    ...
    def _handle_project_summary_failed(self, project_id: str, error_message: str):
        ...
        if (target_history := self._project_context_manager.get_project_history(project_id)) is not None:
            ...
            if self._project_context_manager.get_active_project_id() == project_id: self.new_message_added.emit(error_chat_message)
    ...
    def _cancel_active_tasks(self):
        ...
        if self._modification_coordinator and self._modification_coordinator.is_active(): self._modification_coordinator.cancel_sequence(reason="user_cancel_all")

    def cleanup(self): self._cancel_active_tasks(); self._trigger_save_last_session_state()
    ...
    def _update_rag_initialized_state(self, emit_status: bool = True, project_id: Optional[str] = None):
        ...
        if emit_status or (target_pid == self._project_context_manager.get_active_project_id() and self._rag_initialized != new_init_state): self.update_status_based_on_state()
    ...
    def is_rag_context_initialized(self, project_id: Optional[str]) -> bool:
        ...
        return (self._vector_db_service.is_ready(project_id) and self._vector_db_service.get_collection_size(project_id) > 0)

    def get_project_history(self, project_id: str) -> List[ChatMessage]: return list(self._project_context_manager.get_project_history(project_id) or []) if self._project_context_manager else []
    def get_current_history(self) -> List[ChatMessage]: return list(self._project_context_manager.get_active_conversation_history() or []) if self._project_context_manager else []
    def get_current_project_id(self) -> Optional[str]: return self._project_context_manager.get_active_project_id() if self._project_context_manager else None
    ...
    def _trigger_save_last_session_state(self):
        if self._session_flow_manager:
            ...
            session_extra_data = {"active_chat_backend_id": active_chat_backend_id, "chat_temperature": self._current_chat_temperature, "generator_model_name": self._current_model_names.get(GENERATOR_BACKEND_ID)}
            self._session_flow_manager.save_current_session_to_last_state(self._current_model_names.get(active_chat_backend_id), self._current_chat_personality_prompts.get(active_chat_backend_id), {k: v for k, v in session_extra_data.items() if v is not None})

    def get_current_chat_model(self) -> str: return self._current_model_names.get(self._current_active_chat_backend_id, "Unknown Model")
    def get_current_chat_personality(self) -> Optional[str]: return self._current_chat_personality_prompts.get(self._current_active_chat_backend_id)

    def set_active_chat_backend(self, backend_id: str):
        if backend_id not in self._current_model_names and not any(detail["id"] == backend_id for detail in USER_SELECTABLE_CHAT_BACKEND_DETAILS): self.error_occurred.emit(f"Invalid chat backend type selected: {backend_id}", False); return
        if self._current_active_chat_backend_id != backend_id:
            self._current_active_chat_backend_id = backend_id
            api_key_to_use = get_gemini_api_key() if backend_id.startswith("gemini") else (get_openai_api_key() if backend_id.startswith("gpt") else None)
            if self._backend_coordinator: self._backend_coordinator.configure_backend(backend_id, api_key_to_use, self._current_model_names.get(backend_id, ""), self._current_chat_personality_prompts.get(backend_id))
            self.update_status_based_on_state(); self._trigger_save_last_session_state()

    def set_model_for_backend(self, backend_id: str, model_name: str):
        if backend_id not in self._current_model_names: self.error_occurred.emit(f"Cannot set model for invalid backend: {backend_id}", False); return
        if not model_name: self.error_occurred.emit(f"Model name cannot be empty for backend: {backend_id}", False); return
        self._current_model_names[backend_id] = model_name
        api_key_to_use = get_gemini_api_key() if backend_id.startswith("gemini") else (get_openai_api_key() if backend_id.startswith("gpt") else None)
        if self._backend_coordinator: self._backend_coordinator.configure_backend(backend_id, api_key_to_use, model_name, self._current_chat_personality_prompts.get(backend_id))
        if backend_id == self._current_active_chat_backend_id: self.update_status_based_on_state()
        self._trigger_save_last_session_state()

    def set_personality_for_backend(self, backend_id: str, prompt: Optional[str]):
        if backend_id not in self._current_chat_personality_prompts: self.error_occurred.emit(f"Cannot set personality for invalid backend: {backend_id}", False); return
        self._current_chat_personality_prompts[backend_id] = prompt.strip() if prompt else None
        api_key_to_use = get_gemini_api_key() if backend_id.startswith("gemini") else (get_openai_api_key() if backend_id.startswith("gpt") else None)
        if self._backend_coordinator: self._backend_coordinator.configure_backend(backend_id, api_key_to_use, self._current_model_names.get(backend_id, ""), self._current_chat_personality_prompts[backend_id])
        if backend_id == self._current_active_chat_backend_id: self.update_status_based_on_state()
        self._trigger_save_last_session_state()

    def set_current_project(self, project_id: str):
        if self._project_context_manager and not self._project_context_manager.set_active_project(project_id): self.error_occurred.emit(f"Failed to set project '{project_id}'.", False)
    ...
    def start_new_chat(self):
        if self._session_flow_manager:
            session_extra_data = {"active_chat_backend_id": self._current_active_chat_backend_id, "chat_temperature": self._current_chat_temperature, "generator_model_name": self._current_model_names.get(GENERATOR_BACKEND_ID)}
            self._session_flow_manager.start_new_chat_session(self._current_model_names.get(self._current_active_chat_backend_id), self._current_chat_personality_prompts.get(self._current_active_chat_backend_id), {k: v for k, v in session_extra_data.items() if v is not None})
    def load_chat_session(self, filepath: str):
        if self._session_flow_manager: self._session_flow_manager.load_named_session(filepath, self._current_active_chat_backend_id)
    def save_current_chat_session(self, filepath: str) -> bool:
        if self._session_flow_manager:
            session_extra_data = {"active_chat_backend_id": self._current_active_chat_backend_id, "chat_temperature": self._current_chat_temperature, "generator_model_name": self._current_model_names.get(GENERATOR_BACKEND_ID)}
            return self._session_flow_manager.save_session_as(filepath, self._current_model_names.get(self._current_active_chat_backend_id), self._current_chat_personality_prompts.get(self._current_active_chat_backend_id), {k: v for k, v in session_extra_data.items() if v is not None})
        return False
    def delete_chat_session(self, filepath: str) -> bool: return self._session_flow_manager.delete_named_session(filepath) if self._session_flow_manager else False
    def list_saved_sessions(self) -> List[str]: return self._session_flow_manager.list_saved_sessions() if self._session_flow_manager else []
    def process_user_message(self, text: str, image_data: List[Dict[str, Any]]):
        if self._user_input_handler: self._user_input_handler.handle_user_message(text=text, image_data=image_data, focus_paths=self._current_chat_focus_paths, rag_available=self._rag_available, rag_initialized_for_project=self.is_rag_context_initialized(self.get_current_project_id()))
    def update_status_based_on_state(self):
        active_backend_display_name = next((detail["name"] for detail in USER_SELECTABLE_CHAT_BACKEND_DETAILS if detail['id'] == self._current_active_chat_backend_id), self._current_active_chat_backend_id)
        if not self._chat_backend_configured_successfully.get(self._current_active_chat_backend_id, False):
            ...
            if self._backend_coordinator and (err := self._backend_coordinator.get_last_error_for_backend(self._current_active_chat_backend_id)): err_msg = f"API Error ({active_backend_display_name}): {err}"
            self.status_update.emit(f"{err_msg}. Check settings.", "#e06c75", False, 0)
        elif self._overall_busy: self.status_update.emit(f"Processing with {active_backend_display_name}...", "#e5c07b", False, 0)
        else:
            parts = [f"Ready ({active_backend_display_name})"]
            if self._project_context_manager and (pid := self._project_context_manager.get_active_project_id()) and (pname := self._project_context_manager.get_project_name(pid) or "Unknown"):
                parts.append(f"(Ctx: {constants.GLOBAL_CONTEXT_DISPLAY_NAME if pid == constants.GLOBAL_COLLECTION_ID else pname})")
            if self.is_rag_context_initialized(self._project_context_manager.get_active_project_id() if self._project_context_manager else None): parts.append("[RAG Active]")
    ...
    def set_chat_temperature(self, temperature: float):
        if 0.0 <= temperature <= 2.0:
            ...
            active_backend_display_name = next((detail["name"] for detail in USER_SELECTABLE_CHAT_BACKEND_DETAILS if detail['id'] == self._current_active_chat_backend_id), self._current_active_chat_backend_id)
            self.status_update.emit(f"Temperature for '{active_backend_display_name}' set to {self._current_chat_temperature:.2f}", "#61afef", True, 3000); self._trigger_save_last_session_state()
    ...
    def set_chat_focus(self, paths: List[str]):
        ...
        display_paths = [os.path.basename(p) for p in paths]; display = ", ".join(display_paths[:3]) + (f", ... ({len(display_paths)-3} more)" if len(display_paths) > 3 else "")
        self.status_update.emit(f"Focus set on: {display}", "#61afef", True, 4000)
    def _update_overall_busy_state(self):
        new_busy = (self._backend_coordinator.is_processing_request() if self._backend_coordinator else False) or (self._upload_coordinator.is_busy() if self._upload_coordinator else False)
        if self._overall_busy != new_busy: self._overall_busy = new_busy; self.busy_state_changed.emit(self._overall_busy); self.update_status_based_on_state()
    def is_api_ready(self) -> bool: return self._chat_backend_configured_successfully.get(self._current_active_chat_backend_id, False)